...
<div class="row">
    <div class="col-md-8 col-md-offset-2">
        ...
        <a href="/forum/public" class="btn btn-info pull-right"> {{ __("Volver al listado de los foros") }} </a>
            ...
            <div class="panel panel-default">
                ...
                @if($post->isOwner()) 
                    <div class="panel-footer"> 
                        <form method="POST" action="../posts/{{ $post->slug }}"> // Es necesario enmascarar el método Delete en Laravel 
                            {{ method_field('DELETE') }} 
                            {{ csrf_field() }}
                            <button type="submit" name="deletePost" class="btn btn-danger"> {{ __("Eliminar post") }} 
                            </button> 
                        </form> 
                    </div> 
                @endif
            ...
            @include('partials.errors')
                <form method="POST" action="../posts" enctype="multipart/form-data"> 
                    ...
                    <label class="btn btn-warning" for="file">
                            <input id="file" name="file" type="file" style="display:none;"> {{ __("Subir archivo") }}
                        </label>
